...
int * LineMandelCalculator::calculateMandelbrot () {
	...
	float imag;
	unsigned char maskSum;
	
	#pragma omp simd
	for(size_t i = 0; i < height; i++){
		*((pImags) + i) = y_start + i * dy;
	}

	#pragma omp simd
	for(size_t j = 0; j < width; j++){
		*((pReals) + j) = x_start + j * dx;
	}
	...
	for (i = 0; i < lheight; i++)
	{
		...
		for (k = 0; k < llimit; ++k)
		{
			...
			#pragma omp simd aligned(pRow:64, pReals:64, pzReals:64, pzImags:64, pMask:64) linear(pRow, pReals, pzReals, pzImags, pMask) simdlen(64) reduction(+:maskSum)
			for (j = 0; j < lwidth; j++)
			{
				// if (!(*pMask)){
